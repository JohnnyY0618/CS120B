...
// sound state machine
enum B_States {On, Off} B_State;
void Sound_Tick() {
	unsigned char P_A2 = 0x00; // for store PA2
	P_A2 = ~PINA & 0x04; 
	switch (B_State) {
		case Off:
			if (P_A2) {
...
// global variable control frequency
unsigned char currentFrequncy = 0;
// below state machine control the speaker frequency up and down
enum F_States {F_Start,up,down, A0Release, A1Release} F_State;
void F_Tick() {
...
int main(void) {
	...
	if (S_elapsedTime >= currentFrequncy) { // 2 ms period
		Sound_Tick();	//Execute one tick
		S_elapsedTime = 0;
	}
	F_Tick();
	CombineLEDsSM_Tick();
