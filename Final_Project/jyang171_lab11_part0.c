 ...
 *  Partner(s) Name:
 *	Lab Section:
 *	Assignment: Lab #11  Final Project: Pong Game- "Basic requirements"
...
unsigned char isBegin = 0x00; // for check the game is begin or not
...
int AI_Tick(int state) {
	...
	switch (state) {
		...
		case AI_Start:
			...
			break;
		case AI_init:
			if (moveType == 0x03 && Ball_Pattern == 0x10) {
				if (num % 3 == 0) {
					state = AI_shiftDown;					
				}
				else {
...
// for ball  tatsk
unsigned char isAI = 0x01;  // for check if is AI mode or not
...
int Ball_Tick(int state) {
	...
	if (isAI) { // if is AI mode we against AI paddle
	...
	switch (state) {
		...
		case B_move:
			...
			else if ((Ball_Row == middlePaddleRow) && (Ball_Pattern == 0x02) && (moveType == 0x02)) { // if right and hit middle of paddle
			...
			//test with out second paddle
			//else if ((Ball_Pattern == 0x80) && (moveType == 0x01)) { moveType = 0x02;}
			//test with out 1st paddle
			//else if ((Ball_Pattern == 0x01) && (moveType == 0x02)) { moveType = 0x01;}
...
enum O_States {O_Start,O_init,O_play1, O_play2, O_Ball, O_PauseP1, O_PauseP2, O_PauseB, O_PauseP1R, O_PauseP2R, O_PasueBR, O_PauseDisplay};
...
int main(void) {
	...
	static task taskP,task1,task2,task3,taskOuput;
	task *tasks[] = {&taskP,&task1,&task2,&task3,&taskOuput};
